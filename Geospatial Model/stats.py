...
class TemporalViTModel2(nn.Module):
    def __init__(self, num_classes, img_size=256, patch_size=16, num_frames=1, embed_dim=768, output_embed_dim=768, pretrained='/nfs/turbo/coe-ecbk/jowaria/masked_autoencoder_flowallarc.pth'):
        super().__init__()
        # Encoder
        self.encoder = TemporalViTEncoder(
...
def get_model_predictions(model, test_loader, device):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # Take the most confident class
            _, preds = torch.max(outputs, dim=1)

            all_preds.append(preds.cpu())   # move to CPU
            all_labels.append(labels.cpu()) # move to CPU

    all_preds = torch.cat(all_preds, dim=0)   # shape: [N, H, W]
    all_labels = torch.cat(all_labels, dim=0) # shape: [N, H, W]

    return all_preds.numpy(), all_labels.numpy()

# Load your best model checkpoint
model1 = TemporalViTModel(num_classes=3).to(device)
model2 = TemporalViTModel2(num_classes=3).to(device)
model1.load_state_dict(torch.load('/nfs/turbo/coe-ecbk/jowaria/train_combined_nsra/models/check_44.pth'))

model2.load_state_dict(torch.load('/nfs/turbo/coe-ecbk/jowaria/train_combined_nsra/models/check1_34.pth'))
model1.eval()
model2.eval()

# Get predictions and ground truth
preds_model1, true_labels1 = get_model_predictions(model1, test_loader, device)
preds_model2, true_labels2 = get_model_predictions(model2, test_loader, device)

# Now preds_model and true_labels are ready as numpy arrays


# y_true: true labels
# preds_model1, preds_model2: predictions from each model
# compute_metric: a function to compute precision, recall, f1 etc.
...
for i in range(n_iterations):
    ...
    y_true_sample = true_labels1[indices]
    preds1_sample = preds_model1[indices]
    preds2_sample = preds_model2[indices]

    metrics1 = evaluate_segmentation(preds1_sample, y_true_sample)
    metrics2 = evaluate_segmentation(preds2_sample, y_true_sample)
    
    # Example: compare *mean F1-score* across classes
    metric1 = metrics1['fscore'][:2].mean() 
    #print(metric1)
    metric2 = metrics2['fscore'][:2].mean()
   # print(metric2)
...
# p-value: fraction of times Model 2 was NOT better than Model 1
